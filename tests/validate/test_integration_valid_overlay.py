...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"name": "update", "overlay_file": "tests/samples/simple/update/overlay.yaml"},
        {"name": "remove", "overlay_file": "tests/samples/simple/remove/overlay.yaml"},
        {
            "name": "multi_action",
...
def test_integration_file_based(test_case, capsys):
    ...
    assert (
        result.exit_code == 0
    ), f"Validate command failed for '{test_case['name']}': {result.output}"

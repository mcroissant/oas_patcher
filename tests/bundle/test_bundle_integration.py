...
class TestBundleIntegration:
    ...
    def test_bundle_integration(self, test_case, samples_dir, temp_output_dir):
        ...
        bundle_manager = BundleManager(str(samples_dir))  # Use samples_dir, not bundle_dir
        ...
        if not test_case["should_succeed"]:
            ...
            if test_case["expected_error_type"] == "missing_overlay":
                ...
                assert any("not found" in error for error in validation_result["errors"])
            return
        ...
        for overlay_config in overlays:
            ...
            if overlay_variables:
                overlay_data = template_engine.process_overlay_data(overlay_data, overlay_variables)
    ...
    def _validate_result(self, result_openapi, test_case):
        ...
        if "expected_description" in test_case:
            assert result_openapi["info"]["description"] == test_case["expected_description"]

        # Environment-specific validations
        if "expected_environment" in test_case:
            assert result_openapi["info"]["x-environment"] == test_case["expected_environment"]
        ...
        if test_case.get("expected_has_api_key"):
            security_schemes = result_openapi.get("components", {}).get("securitySchemes", {})
    ...
    BUNDLE_DISCOVERY_TEST_CASES = [
        {
            ...
            "expected_bundles": ["simple_bundle", "multi_overlay_bundle", "environment_bundle", 
                              "template_bundle", "invalid_bundle"]
        }
    ]

    @pytest.mark.parametrize("test_case", BUNDLE_DISCOVERY_TEST_CASES, ids=lambda tc: tc["id"])
    ...
    @pytest.mark.parametrize("test_case", BUNDLE_VALIDATION_TEST_CASES, ids=lambda tc: tc["id"])
    ...
    @pytest.mark.parametrize("test_case", BUNDLE_INFO_TEST_CASES, ids=lambda tc: tc["id"])
    def test_bundle_info(self, test_case, samples_dir):
        ...
        if test_case.get("expected_has_environment_overlays"):
            environment_overlays = [
                overlay for overlay in bundle_info["overlays"]
    ...
    def test_environment_overlay_filtering(self, samples_dir):
        ...
        dev_overlays = bundle_manager.get_overlays_for_environment("environment_bundle", "dev")
        ...
        assert "prod-overlay.yaml" not in dev_overlay_paths  # Should not include prod overlay

        # Test production environment
        prod_overlays = bundle_manager.get_overlays_for_environment("environment_bundle", "prod")
        ...
        assert "dev-overlay.yaml" not in prod_overlay_paths   # Should not include dev overlay

        # Test non-existent environment
        unknown_overlays = bundle_manager.get_overlays_for_environment("environment_bundle", "unknown")
        unknown_overlay_paths = [overlay.path for overlay in unknown_overlays]
        assert "base-overlay.yaml" in unknown_overlay_paths  # Should include base overlay
    ...
    def test_template_processing_integration(self, samples_dir):
        ...
        overlay_path = bundle_manager._resolve_overlay_path("template_bundle", "template-overlay.yaml")
        overlay_data = load_file(str(overlay_path))
        
        # Process templates
        processed_overlay = template_engine.process_overlay_data(overlay_data, variables)
        
        # Verify template processing
        info_action = next(action for action in processed_overlay["actions"] if action["target"] == "$.info")
...
class TestBundleAdvancedScenarios:
    ...
    def test_complex_overlay_chain(self, tmp_path):
        ...
        base_overlay = {
            ...
            "actions": [
                {"target": "$.info", "update": {"title": "{{ app_name }}", "x-stage": "{{ stage }}"}}
            ]
        }
        ...
        middleware_overlay = {
            ...
            "actions": [
                {"target": "$.info", "update": {"description": "Processed in {{ stage }}"}}
            ]
        }
        ...
        final_overlay = {
            ...
            "actions": [
                {"target": "$.info", "update": {"version": "{{ version }}", "x-final-stage": "{{ stage }}"}}
            ]
        }
        ...
        # Apply bundle
        bundle_manager = BundleManager(str(tmp_path))  # Use parent directory, not bundle_dir
        template_engine = TemplateEngine()
        ...
        for overlay_config in bundle_config.overlays:
            overlay_path = bundle_manager._resolve_overlay_path("complex_bundle", overlay_config.path)
            overlay_data = load_file(str(overlay_path))

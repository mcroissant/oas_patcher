import pytest
from oas_patch.overlay import apply_overlay, deep_update


def test_apply_overlay_update_action():
    """Test applying an overlay with update actions."""
    openapi_doc = {"paths": {"/example": {"get": {"summary": "Original summary"}}}}
    overlay = {
        "actions": [
            {
...
def test_apply_overlay_remove_action():
    ...
    openapi_doc = {"paths": {"/example": {"get": {"summary": "Original summary"}}}}
    overlay = {"actions": [{"target": "$.paths['/example'].get", "remove": True}]}
    result = apply_overlay(openapi_doc, overlay)
...
def test_deep_update_nested_keys():
    ...
    target = {"key1": {"subkey1": "value1"}}
    updates = {"key1": {"subkey2": "value2"}}
...
def test_deep_update_overwrite_keys():
    ...
    target = {"key1": {"subkey1": "value1"}}
    updates = {"key1": {"subkey1": "new_value1"}}
    deep_update(target, updates)
        ...
        "actions": [
            {
                ...
                "update": {"description": "This is a root-level description"},
            }
        ]
    }
    result = apply_overlay(openapi_doc, overlay)
...
def test_apply_overlay_remove_root():
    ...
    overlay = {"actions": [{"target": "$", "remove": True}]}  # Root of the document
...
def test_apply_overlay_non_dict_update_root():
    ...
    overlay = {
        "actions": [
            {"target": "$", "update": "Invalid root update"}  # Root of the document
        ]
    }
    with pytest.raises(
        ValueError, match="Cannot perform non-dict update on the root of the document"
    ):
        apply_overlay(openapi_doc, overlay)
...
def test_apply_overlay_merge_dict_with_array():
    ...
    openapi_doc = {
        ...
        "paths": {
            "/example": {
                "get": {"summary": "Original summary", "security": [{"api_key": []}]}
            }
        },
    }
    overlay = {
    ...
    result = apply_overlay(openapi_doc, overlay)
    assert (
        len(result["paths"]["/example"]["get"]["security"]) == 2
    )  # Ensure both security definitions are present

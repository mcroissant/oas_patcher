...
def test_cli_output_to_file(setup_mocks, mock_save_file, mock_load_file, mock_apply_overlay):
    """Test the CLI with output to a file."""
    result = run_cli_with_args(['overlay', 'openapi.yaml', 'overlay.yaml', '-o', 'output.yaml'])
...
def test_cli_output_to_console(setup_mocks, mock_load_file, mock_apply_overlay):
    ...
    assert yaml.safe_load(result.output) == {"openapi": "3.0.3", "info": {"title": "Updated API"}}
...
def test_cli_with_sanitize_flag(setup_mocks, mock_load_file, mock_apply_overlay):
    ...
    result = run_cli_with_args(['overlay', 'openapi.yaml', 'overlay.yaml', '--sanitize'])
...
def test_apply_command_basic_success(mock_enhanced_features, mock_enhanced_file_operations):
    ...
    mock_enhanced_features['bundle_manager'].return_value.load_bundle.return_value = bundle_config
    mock_enhanced_features['env_manager'].return_value.get_variables.return_value = {}
    mock_enhanced_features['bundle_manager'].return_value._resolve_overlay_path.return_value = 'test.yaml'
    ...
    mock_enhanced_file_operations['apply_overlay'].return_value = {'openapi': '3.0.0', 'info': {'title': 'Modified API'}}
    mock_enhanced_features['template_engine'].return_value.process_overlay_data.return_value = {'overlay': '1.0.0', 'actions': []}
    ...
    mock_enhanced_features['bundle_manager'].return_value.load_bundle.assert_called_once_with('test-bundle')


def test_apply_command_with_environment(mock_enhanced_features, mock_enhanced_file_operations):
    ...
    mock_enhanced_features['bundle_manager'].return_value.load_bundle.return_value = bundle_config
    mock_enhanced_features['bundle_manager'].return_value.get_overlays_for_environment.return_value = bundle_config.overlays
    mock_enhanced_features['env_manager'].return_value.get_variables.return_value = {'env_var': 'env_value'}
    mock_enhanced_features['bundle_manager'].return_value._resolve_overlay_path.return_value = 'test.yaml'
    ...
    assert result.exit_code == 0
    mock_enhanced_features['bundle_manager'].return_value.get_overlays_for_environment.assert_called_once_with('test-bundle', 'staging')


def test_apply_command_with_variables(mock_enhanced_features, mock_enhanced_file_operations):
    """Test apply command with CLI variables."""
    # Setup mocks
    from unittest.mock import MagicMock
    
    bundle_config = MagicMock()
    bundle_config.overlays = [MagicMock(path='test.yaml', variables={'overlay_var': 'overlay_value'})]
    ...
    result = run_cli_with_args([
        ...
        'openapi.yaml',
        'test-bundle',
        '--var', 'cli_var=cli_value',
        '--var', 'another_var=another_value',
        '--output', 'output.yaml'
    ])

    assert result.exit_code == 0
    mock_enhanced_features['template_engine'].return_value.process_overlay_data.assert_called()
...
def test_apply_command_missing_openapi_file(mock_enhanced_features, mock_enhanced_file_operations):
    """Test apply command with missing OpenAPI file."""
    mock_enhanced_file_operations['load_file'].side_effect = FileNotFoundError("File not found")
...
def test_apply_command_missing_bundle(mock_enhanced_features, mock_enhanced_file_operations):
    """Test apply command with missing bundle."""
    mock_enhanced_file_operations['load_file'].return_value = {'openapi': '3.0.0'}
    mock_enhanced_features['bundle_manager'].return_value.load_bundle.side_effect = FileNotFoundError("Bundle not found")
...
def test_list_bundles_command_basic(mock_enhanced_features):
    ...
    mock_enhanced_features['bundle_manager'].return_value.discover_bundles.return_value = ['bundle1', 'bundle2', 'bundle3']

    result = run_cli_with_args(['list-bundles'])

    assert result.exit_code == 0
    mock_enhanced_features['cli_utils'].print_header.assert_called_once_with("Available Bundles")
...
def test_list_bundles_command_verbose(mock_enhanced_features):
    ...
    mock_enhanced_features['bundle_manager'].return_value.discover_bundles.return_value = ['bundle1']
    mock_enhanced_features['bundle_manager'].return_value.get_bundle_info.return_value = mock_bundles_info[0]
...
def test_list_bundles_command_no_bundles(mock_enhanced_features):
    ...
    mock_enhanced_features['bundle_manager'].return_value.discover_bundles.return_value = []

    result = run_cli_with_args(['list-bundles'])

    assert result.exit_code == 0
    mock_enhanced_features['cli_utils'].print_info.assert_called_with("No bundles found in overlays")


def test_list_bundles_command_error(mock_enhanced_features):
    """Test list-bundles command with error."""
    mock_enhanced_features['bundle_manager'].return_value.discover_bundles.side_effect = Exception("Failed to discover bundles")
...
def test_list_environments_command_basic(mock_enhanced_features):
    ...
    mock_enhanced_features['env_manager'].return_value.discover_environments.return_value = ['dev', 'staging', 'prod']

    result = run_cli_with_args(['list-environments'])

    assert result.exit_code == 0
    mock_enhanced_features['cli_utils'].print_header.assert_called_once_with("Available Environments")
...
def test_list_environments_command_verbose(mock_enhanced_features):
    ...
    mock_env_info = [
        {'name': 'dev', 'description': 'Development environment', 'variables': {'debug': 'true'}},
        {'name': 'prod', 'description': 'Production environment', 'variables': {'debug': 'false'}}
    ]
    
    mock_enhanced_features['env_manager'].return_value.list_environments_info.return_value = mock_env_info
...
def test_list_environments_command_no_environments(mock_enhanced_features):
    ...
    mock_enhanced_features['env_manager'].return_value.discover_environments.return_value = []

    result = run_cli_with_args(['list-environments'])

    assert result.exit_code == 0
    mock_enhanced_features['cli_utils'].print_info.assert_called_with("No environments found in overlays")
...
def test_bundle_validate_command_success(mock_enhanced_features):
    ...
    mock_enhanced_features['bundle_manager'].return_value.validate_bundle.return_value = mock_validation_result

    result = run_cli_with_args(['bundle-validate', 'test-bundle'])

    assert result.exit_code == 0
    mock_enhanced_features['bundle_manager'].return_value.validate_bundle.assert_called_once_with('test-bundle')
...
def test_bundle_validate_command_exception(mock_enhanced_features):
    ...
    mock_enhanced_features['bundle_manager'].return_value.validate_bundle.side_effect = Exception("Validation error")
...
def test_info_command_success(mock_enhanced_features):
    ...
    mock_enhanced_features['bundle_manager'].return_value.get_bundle_info.return_value = mock_bundle_info

    result = run_cli_with_args(['info', 'test-bundle'])

    assert result.exit_code == 0
    mock_enhanced_features['bundle_manager'].return_value.get_bundle_info.assert_called_once_with('test-bundle')
    mock_enhanced_features['cli_utils'].console.print.assert_called()


def test_info_command_exception(mock_enhanced_features):
    """Test info command with exception."""
    mock_enhanced_features['bundle_manager'].return_value.get_bundle_info.side_effect = Exception("Bundle not found")
...
def test_init_command_new_directory(mock_enhanced_features, mocker):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert mock_yaml_dump.call_count >= 4  # bundle.yaml + 2 overlays + 2 environments
...
def test_init_command_existing_directory_no_force(mock_enhanced_features, mocker):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert result.exit_code == 0
        mock_enhanced_features['cli_utils'].print_info.assert_called_with("Initialization cancelled")
...
def test_init_command_exception(mock_enhanced_features, mocker):
    ...
    mocker.patch('oas_patch.oas_patcher_cli.Path').side_effect = Exception("Permission denied")
...
def test_parse_cli_variables_function():
    ...
    variables = parse_cli_variables(['key1=value1', 'key2=value2', 'key3=value with spaces'])

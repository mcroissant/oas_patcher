...
def parse_cli_variables(var_strings):
    ...
    for var_string in var_strings:
        if '=' not in var_string:
            raise click.BadParameter(f"Variable must be in format key=value: {var_string}")
        key, value = var_string.split('=', 1)
        variables[key.strip()] = value.strip()
    return variables


def auto_generate_output_filename(input_file: str, bundle_name: str, output_format: str) -> str:
...
@click.option('-o', '--output', help='Path to save the modified OpenAPI document. Defaults to stdout.')
@click.option('--sanitize', is_flag=True, help='Remove special characters from the OpenAPI document.')
def overlay(openapi, overlay, output, sanitize):
    ...
    if output:
        ...
        if openapi.endswith(('.yaml', '.yml')):
            ...
            click.echo(yaml.dump(modified_doc, sort_keys=False, default_flow_style=False))
...
def diff(original, modified, output):
    ...
    if output:
        ...
        if original.endswith(('.yaml', '.yml')):
            yaml.Dumper.ignore_aliases = lambda *args: True
            click.echo(yaml.dump(overlay_doc, sort_keys=False, default_flow_style=False))
...
@click.option('--format', type=click.Choice(['sh', 'log', 'yaml']), default='sh',
...
@click.option('--config', '-c', default='overlays', help='Configuration directory (default: overlays)')
@click.option('--format', '-f', type=click.Choice(['yaml', 'json']), help='Output format')
...
@click.option('--verbose', '-v', is_flag=True, help='Verbose output with progress indicators')
def apply(openapi_file, bundle_name, output, env, config, format, var, dry_run, verbose):
    """
    ...
    try:
        ...
        if not output:
            output = auto_generate_output_filename(openapi_file, bundle_name, output_format)
        ...
        with cli_utils.create_progress_context("Applying overlays") as progress:
            ...
            for overlay_config in overlays:
                ...
                # Load overlay file
                overlay_path = bundle_manager._resolve_overlay_path(bundle_name, overlay_config.path)
                try:
                    overlay_data = load_file(str(overlay_path))
                except (FileNotFoundError, ValueError) as e:
                    cli_utils.print_error(f"Failed to load overlay {overlay_config.path}: {e}")
                    continue
                ...
                try:
                    ...
                    cli_utils.print_error(f"Template processing failed for {overlay_config.path}: {e}")
                    continue
                ...
                try:
                    ...
                    overlays_applied += 1
                    if verbose:
                        cli_utils.print_success(f"Applied overlay: {overlay_config.path}")
                except Exception as e:
                    cli_utils.print_error(f"Failed to apply overlay {overlay_config.path}: {e}")
                    continue
...
def init(config, force):
    ...
    try:
        ...
        if config_path.exists() and not force:
            if not cli_utils.confirm_action(f"Configuration directory '{config}' already exists. Continue?"):
        ...
        server_overlay = {
            'overlay': '1.0.0',
            'info': {
                'title': 'Example Server Configuration'
            },
            'actions': [
                {
                    'target': '$',
        ...
        cli_utils.print_info(f"  oas-patch bundle-validate example-bundle --config {config}")

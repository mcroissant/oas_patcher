...
class EnvironmentManager:
    ...
    def load_environment(self, env_name: str) -> EnvironmentConfig:
        ...
        if not env_path:
            raise FileNotFoundError(f"Environment '{env_name}' not found in {self.config_dir}")
    ...
    def _parse_environment_config(self, config_data: Dict[str, Any], env_name: str) -> EnvironmentConfig:
    ...
    def get_variables(self, env_name: Optional[str], cli_variables: Optional[Dict[str, str]] = None) -> Dict[str, Any]:
    ...
    def validate_environment(self, env_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not env_config.variables:
                validation_result['warnings'].append("Environment contains no variables")
            
            # Validate variable types
            for key, value in env_config.variables.items():
                if not isinstance(key, str):
                    validation_result['errors'].append(f"Variable key must be string: {key}")

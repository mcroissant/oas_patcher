...
class TemplateEngine:
    ...
    def process_overlay_data(self, overlay_data: Dict[str, Any], variables: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def validate_template(self, content: str, variables: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    def _resolve_env_vars_in_variables(self, variables: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in resolved_vars.items():
            if isinstance(value, str):
                ...
                resolved_vars[key] = [
                    self._resolve_env_vars_in_value(item) if isinstance(item, str) 
                    else self._resolve_env_vars_recursive(item) if isinstance(item, dict)
                    else item 
                    for item in value
                ]
    ...
    def _resolve_env_vars_recursive(self, data: Any) -> Any:
        ...
        if isinstance(data, str):
            ...
            return {key: self._resolve_env_vars_recursive(value) for key, value in data.items()}
        elif isinstance(data, list):

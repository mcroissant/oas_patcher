...
class CLIUtils:
    ...
    def create_bundles_table(self, bundles_info: List[Dict[str, Any]]) -> Table:
        ...
        for bundle_info in bundles_info:
            status = "✓ Valid" if bundle_info.get('validation', {}).get('valid', False) else "✗ Invalid"
            status_style = "green" if bundle_info.get('validation', {}).get('valid', False) else "red"
    ...
    def create_environments_table(self, environments_info: List[Dict[str, Any]]) -> Table:
        ...
        for env_info in environments_info:
            status = "✓ Valid" if env_info.get('validation', {}).get('valid', False) else "✗ Invalid"
            status_style = "green" if env_info.get('validation', {}).get('valid', False) else "red"
    ...
    def create_bundle_info_panel(self, bundle_info: Dict[str, Any]) -> Panel:
        ...
        content_lines.append(f"[bold]Version:[/bold] {bundle_info.get('version', 'Unknown')}")
        content_lines.append(f"[bold]Description:[/bold] {bundle_info.get('description', 'No description')}")
        ...
        for i, overlay in enumerate(overlays, 1):
            env_info = f" (env: {', '.join(overlay.get('environment', []))})" if overlay.get('environment') else ""
        ...
        content = "\n".join(content_lines)
        return Panel(content, title=f"Bundle: {bundle_info.get('name', 'Unknown')}", border_style="blue")
    ...
    def print_validation_results(self, validation: Dict[str, Any], title: str = "Validation Results"):
    ...
    def print_overlay_application_summary(self, input_file: str, output_file: str, 
                                        bundle_name: str, overlays_applied: int):
